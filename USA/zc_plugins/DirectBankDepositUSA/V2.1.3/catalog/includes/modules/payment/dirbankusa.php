<?php
/**
 * Direct Bank Deposit - USA payment module (dirbankusa).
 *
 ...
 * V2.1.2  2025-12-20  updated for zc220, PHP 8.2 to PHP 8.4
 * V2.1.2a 2026-02-27
 * V2.1.3  2026-08-25  One-Page Checkout integration (bottom-instructions observer)
 */

declare(strict_types=1);
if (!defined('VERSION_USA')) {   define('VERSION_USA', '2.1.3'); }
...
if (!defined('MODULE_PAYMENT_DIRBANKUS_TEXT_TITLE')) {
    $filename = "dirbankusa.php";
...
// class constructor
    function __construct() {
      ...
      $this->description = 'V' . VERSION_USA . ' ' .MODULE_PAYMENT_DIRBANKUSA_TEXT_DESCRIPTION;  // show version in admin panel
      $this->email_footer = defined('MODULE_PAYMENT_DIRBANKUSA_TEXT_EMAIL_FOOTER');
      $this->sort_order = defined('MODULE_PAYMENT_DIRBANKUSA_SORT_ORDER') ? ((int)MODULE_PAYMENT_DIRBANKUSA_SORT_ORDER) : NULL;
...
// disable the module if the order only contains virtual products
    ...
    function install() {
        ...
        if (defined('MODULE_PAYMENT_DIRBANKUSA_STATUS')) {
          $messageStack->add_session('DirBankUS module already installed.', 'error');
          zen_redirect(zen_href_link(FILENAME_MODULES, 'set=payment&module=dirbankus', 'NONSSL'));
          return 'failed';
        }
     $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key,
        configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Enable Direct Bank Deposit Module',
        'MODULE_PAYMENT_DIRBANKUSA_STATUS', 'True', 'Do you want to accept USA Bank Deposit payments?', '6', '1',
        'zen_cfg_select_option(array(\'True\', \'False\'), ', now())");
	 $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key,
        configuration_value, configuration_description, configuration_group_id, sort_order, use_function, set_function,
        date_added) values ('Payment Zone', 'MODULE_PAYMENT_DIRBANKUSA_ZONE', '0', 'If a zone is selected, only enable this payment method for that zone.',
     ...
     $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key,
        ...
        'MODULE_PAYMENT_DIRBANKUSA_SORT_ORDER', '0', 'Sort order of display. Lowest is displayed first.', '6', '0', now())");
     $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key,
        configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Routing Number',
        'MODULE_PAYMENT_DIRBANKUSA_ROUTING', '000-000-000', '9 digit routing number in the format 000-000-000', '6', '1', now());");
     $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key,
        configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Bank Account No.',
        'MODULE_PAYMENT_DIRBANKUSA_ACCNUM', '12345678', 'Bank Account No.', '6', '1', now());");
     $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key,
        configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Bank Account Name',
        'MODULE_PAYMENT_DIRBANKUSA_ACCNAM', 'Joe Bloggs', 'Bank Account Name', '6', '1', now());");
     $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key,
        configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Bank Name',
        'MODULE_PAYMENT_DIRBANKUSA_BANKNAM', 'The Bank', 'Bank Name', '6', '1', now());");
     $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key,
        configuration_value, configuration_description, configuration_group_id, sort_order, set_function, use_function,
        date_added) values ('Set Order Status', 'MODULE_PAYMENT_DIRBANKUSA_ORDER_STATUS_ID', '0',
        'Set the status of orders made with this payment module to this value', '6', '0', 'zen_cfg_pull_down_order_statuses(', 'zen_get_order_status_name', now())");
   }
