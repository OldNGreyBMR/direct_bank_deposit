...
* @version      $Id V2.1.3 dirbankaus 2026-08-25  BMH (OldNGrey) for Zen Cart v2.2.x / OPC v2.4+ (v2.7.0 tested) PHP 8.2 to PHP 8.5
*/
/**
 ...
 * while the customer has the dirbankaus payment-method radio selected; choosing another
...
class dirbankaus_opc_observer extends base
{
    protected string $module_code = 'dirbankaus';
    ...
    protected function defineOpcBottomInstructions(): void
    {
        ...
        //
        if ((string)zen_config('MODULE_PAYMENT_DIRBANKAUS_STATUS', 'False') !== 'True') {
            return;
        }
    ...
    {
        $geo_zone = (int)zen_config('MODULE_PAYMENT_DIRBANKAUS_ZONE', 0);
        if ($geo_zone <= 0) {
            return true;
        }
    ...
    {
        $accnum = htmlspecialchars((string)zen_config('MODULE_PAYMENT_DIRBANKAUS_ACCNUM', ''), ENT_QUOTES);
        $bsb    = htmlspecialchars((string)zen_config('MODULE_PAYMENT_DIRBANKAUS_BSB', ''), ENT_QUOTES);
        $accnam = htmlspecialchars((string)zen_config('MODULE_PAYMENT_DIRBANKAUS_ACCNAM', ''), ENT_QUOTES);
        $banknam= htmlspecialchars((string)zen_config('MODULE_PAYMENT_DIRBANKAUS_BANKNAM', ''), ENT_QUOTES);
        if ($accnum === '' && $bsb === '') {
            return '';
        }
        ...
        $title = defined('MODULE_PAYMENT_DIRBANKAUS_TEXT_TITLE') ? MODULE_PAYMENT_DIRBANKAUS_TEXT_TITLE : 'Direct Bank Deposit';

        // Pre-show the block only if dirbankaus was previously selected (page re-entry).
        $display = (($_SESSION['payment'] ?? '') === $this->module_code) ? 'block' : 'none';

        $html  = '<span id="dbd-opc-bank-details" style="display:' . $display . ';margin-top:0.5rem;">';
        ...
        $html .= 'var box=$("#dbd-opc-bank-details");';
