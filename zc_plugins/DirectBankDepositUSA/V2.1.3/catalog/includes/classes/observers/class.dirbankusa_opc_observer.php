<?php
/**
 * Direct Bank Deposit - USA (dirbankusa) : One-Page Checkout (OPC) integration.
 *
 ...
 * while the customer has the dirbankusa payment-method radio selected; choosing another
...
class dirbankusa_opc_observer extends base
{
    protected string $module_code = 'dirbankusa';
    ...
    protected function defineOpcBottomInstructions(): void
    {
        ...
        //
        if ((string)zen_config('MODULE_PAYMENT_DIRBANKUSA_STATUS', 'False') !== 'True') {
            return;
        }
    ...
    {
        $geo_zone = (int)zen_config('MODULE_PAYMENT_DIRBANKUSA_ZONE', 0);
        if ($geo_zone <= 0) {
            return true;
        }
    ...
    {
        $accnum   = htmlspecialchars((string)zen_config('MODULE_PAYMENT_DIRBANKUSA_ACCNUM', ''), ENT_QUOTES);
        $routing  = htmlspecialchars((string)zen_config('MODULE_PAYMENT_DIRBANKUSA_ROUTING', ''), ENT_QUOTES);
        $accnam   = htmlspecialchars((string)zen_config('MODULE_PAYMENT_DIRBANKUSA_ACCNAM', ''), ENT_QUOTES);
        $banknam  = htmlspecialchars((string)zen_config('MODULE_PAYMENT_DIRBANKUSA_BANKNAM', ''), ENT_QUOTES);
        if ($accnum === '' && $routing === '') {
            return '';
        }
        ...
        $title = defined('MODULE_PAYMENT_DIRBANKUSA_TEXT_TITLE') ? MODULE_PAYMENT_DIRBANKUSA_TEXT_TITLE : 'Direct Bank Deposit';

        // Pre-show the block only if dirbankusa was previously selected (page re-entry).
        $display = (($_SESSION['payment'] ?? '') === $this->module_code) ? 'block' : 'none';

        $html  = '<span id="dbus-opc-bank-details" style="display:' . $display . ';margin-top:0.5rem;">';
        ...
        $html .= 'Routing Number: ' . $routing . '<br>';
        ...
        $html .= 'var box=$("#dbus-opc-bank-details");';
