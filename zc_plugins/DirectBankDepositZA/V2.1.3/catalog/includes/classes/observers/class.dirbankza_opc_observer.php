<?php
/**
 * Direct Bank Deposit - ZA (dirbankza) : One-Page Checkout (OPC) integration.
 *
 ...
 * while the customer has the dirbankza payment-method radio selected; choosing another
...
class dirbankza_opc_observer extends base
{
    protected string $module_code = 'dirbankza';
    ...
    protected function defineOpcBottomInstructions(): void
    {
        ...
        //
        if ((string)zen_config('MODULE_PAYMENT_DIRBANKZA_STATUS', 'False') !== 'True') {
            return;
        }
    ...
    {
        $geo_zone = (int)zen_config('MODULE_PAYMENT_DIRBANKZA_ZONE', 0);
        if ($geo_zone <= 0) {
            return true;
        }
    ...
    {
        $accnam   = htmlspecialchars((string)zen_config('MODULE_PAYMENT_DIRBANKZA_ACCNAM', ''), ENT_QUOTES);
        $accnum   = htmlspecialchars((string)zen_config('MODULE_PAYMENT_DIRBANKZA_ACCNUM', ''), ENT_QUOTES);
        $banknam  = htmlspecialchars((string)zen_config('MODULE_PAYMENT_DIRBANKZA_BANKNAM', ''), ENT_QUOTES);
        $bankcode = htmlspecialchars((string)zen_config('MODULE_PAYMENT_DIRBANKZA_BANKCODE', ''), ENT_QUOTES);
        if ($accnum === '' && $bankcode === '') {
            return '';
        }
        ...
        $title = defined('MODULE_PAYMENT_DIRBANKZA_TEXT_TITLE') ? MODULE_PAYMENT_DIRBANKZA_TEXT_TITLE : 'Direct Bank Deposit';

        // Pre-show the block only if dirbankza was previously selected (page re-entry).
        $display = (($_SESSION['payment'] ?? '') === $this->module_code) ? 'block' : 'none';

        $html  = '<span id="dbza-opc-bank-details" style="display:' . $display . ';margin-top:0.5rem;">';
        ...
        $html .= 'var box=$("#dbza-opc-bank-details");';
