<?php
/**
 * Direct Bank Deposit - NZ (dirbanknz) : One-Page Checkout (OPC) integration.
 *
 ...
 * while the customer has the dirbanknz payment-method radio selected; choosing another
...
class dirbanknz_opc_observer extends base
{
    protected string $module_code = 'dirbanknz';
    ...
    protected function defineOpcBottomInstructions(): void
    {
        ...
        //
        if ((string)zen_config('MODULE_PAYMENT_DIRBANKNZ_STATUS', 'False') !== 'True') {
            return;
        }
    ...
    {
        $geo_zone = (int)zen_config('MODULE_PAYMENT_DIRBANKNZ_ZONE', 0);
        if ($geo_zone <= 0) {
            return true;
        }
    ...
    {
        $branchnum = htmlspecialchars((string)zen_config('MODULE_PAYMENT_DIRBANKNZ_BRANCHNUM', ''), ENT_QUOTES);
        $accnum    = htmlspecialchars((string)zen_config('MODULE_PAYMENT_DIRBANKNZ_ACCNUM', ''), ENT_QUOTES);
        $accnam    = htmlspecialchars((string)zen_config('MODULE_PAYMENT_DIRBANKNZ_ACCNAM', ''), ENT_QUOTES);
        $banknam   = htmlspecialchars((string)zen_config('MODULE_PAYMENT_DIRBANKNZ_BANKNAM', ''), ENT_QUOTES);
        if ($accnum === '' && $branchnum === '') {
            return '';
        }
        ...
        $title = defined('MODULE_PAYMENT_DIRBANKNZ_TEXT_TITLE') ? MODULE_PAYMENT_DIRBANKNZ_TEXT_TITLE : 'Direct Bank Deposit';

        // Pre-show the block only if dirbanknz was previously selected (page re-entry).
        $display = (($_SESSION['payment'] ?? '') === $this->module_code) ? 'block' : 'none';

        $html  = '<span id="dbnz-opc-bank-details" style="display:' . $display . ';margin-top:0.5rem;">';
        ...
        $html .= 'var box=$("#dbnz-opc-bank-details");';
