...
* @copyright    Portions (c) 2003 osCommerce
* @license      http://www.zen-cart.com/license/2_0.txt GNU Public License V2.0
* @version      $Id: DIRBANKAUS.php 1106 2009-11-24 22:05:35Z CRYSTAL JONES $ modify from Auzbank of OZcommerce module by birdbrain
* @version      $Id V2.1.3 dirbankaus.php 2026-08-25  BMH (OldNGrey) for Zen Cart v2.2.x / OPC v2.4+ (v2.7.0 tested) PHP 8.2 to PHP 8.5
*
*/
// integration with One Page Checkout (OPC) to display bank account details in
// "Bottom Instructions" box on checkout screen.

declare(strict_types = 1);
define('MODULE_PAYMENT_DIRBANKAUS_VERSION_AUS','2.1.2a');
...
// class constructor
    function __construct() {
      ...
      $this->title = defined('MODULE_PAYMENT_DIRBANKAUS_TEXT_TITLE') ? MODULE_PAYMENT_DIRBANKAUS_TEXT_TITLE : 'Direct-Bank-Deposit - AU'; // Default title if constant is not defined

      $this->description = 'V' . MODULE_PAYMENT_DIRBANKAUS_VERSION_AUS . ' ' . MODULE_PAYMENT_DIRBANKAUS_TEXT_DESCRIPTION; // show version in admin panel
    ...
    if (null === $this->sort_order)   return;   // BMH try without return value if (null === $this->sort_order) return false;
...
// disable the module if the order only contains virtual products
    ...
    function install() {
     ...
     (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values
      ('Enable Direct Bank Deposit/Cheque Module', 'MODULE_PAYMENT_DIRBANKAUS_STATUS',
      'True', 'Do you want to accept Australian Bank Deposit payments, cheques and money orders?', '6', '1', 'zen_cfg_select_option(array(\'True\', \'False\'), ', now())");
    ...
    $db->Execute("insert into " . TABLE_CONFIGURATION .
        ...
        ('Payment Zone', 'MODULE_PAYMENT_DIRBANKAUS_ZONE',
        '0',  'If a zone is selected, only enable this payment method for that zone.', '6', '2', 'zen_get_zone_class_title', 'zen_cfg_pull_down_zone_classes(', now())");

     $db->Execute("insert into " . TABLE_CONFIGURATION .
        " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added)
        values ('Sort order of display.', 'MODULE_PAYMENT_DIRBANKAUS_SORT_ORDER',
        '10',  'Sort order of display. Lowest is displayed first.', '6', '0', now())");

     $db->Execute("insert into " . TABLE_CONFIGURATION .
        " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added)
        values ('BSB Number', 'MODULE_PAYMENT_DIRBANKAUS_BSB',
        '123-456',        'BSB Number in the format 000-000', '6', '1', now());");

     $db->Execute("insert into " . TABLE_CONFIGURATION .
        " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added)
        values ('Bank Account No.', 'MODULE_PAYMENT_DIRBANKAUS_ACCNUM',
        '12345678',      'Bank Account No.', '6', '1', now());");

     $db->Execute("insert into " . TABLE_CONFIGURATION .
        " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added)
        values ('Swift Code.', 'MODULE_PAYMENT_DIRBANKAUS_SWIFT',
        '12345678', 'Swift Code.', '6', '1', now());");

     $db->Execute("insert into " . TABLE_CONFIGURATION .
        " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added)
        values ('Bank Account Name', 'MODULE_PAYMENT_DIRBANKAUS_ACCNAM',
        'Account Name', 'Bank Account Name', '6', '1', now());");

     $db->Execute("insert into " . TABLE_CONFIGURATION .
        " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added)
        values ('Bank Name', 'MODULE_PAYMENT_DIRBANKAUS_BANKNAM',
        ' Your Bank Name', 'Bank Name', '6', '1', now());");
     ...
     $db->Execute("insert into " . TABLE_CONFIGURATION .
        ...
        ('Set Order Status', 'MODULE_PAYMENT_DIRBANKAUS_ORDER_STATUS_ID',
         '0', 'Set the status of orders made with this payment module to this value', '6', '0', 'zen_cfg_pull_down_order_statuses(', 'zen_get_order_status_name', now())");
     ...
     $db->Execute("insert into " . TABLE_CONFIGURATION .
        ...
        ('Address', 'MODULE_PAYMENT_DIRBANKAUS_ADDRESS',
        '44 Australia St. Brisbane 4000','Address to send cheques/money orders.','6','1',now());");

     $db->Execute("insert into " . TABLE_CONFIGURATION .
        " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values
        ('Payable', 'MODULE_PAYMENT_DIRBANKAUS_PAYABLE',
        'Your Store Name','Cheques/Money Orders Payable to:','6','1',now());");
    }
